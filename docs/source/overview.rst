...
- the allowed source-sink relations are encoded in
  ``source_sink_matrix``
- :math:`\hat{S}_i` is a prepared source image for channel :math:`i`
...
with one optional background offset :math:`\beta_{ij}` per modeled
...
jointly by default. For a sink channel :math:`j`, the optimization problem is:
...
preprint:
   ...
   Musacchio, F., & Fuhrmann, M. (2026). *Spectral Unmixing: A modular and
   reproducible Python package for directed and blind spectral unmixing in
   multidimensional microscopy stacks*. bioRxiv.
   https://doi.org/10.64898/2026.07.06.736825
